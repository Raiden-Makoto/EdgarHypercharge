...
class MolTreeNode(object):
    """Node in a molecular tree representing a clique in the molecule."""

    def __init__(self, smiles, clique=None):
        """Initialize a molecular tree node."""
        self.smiles = smiles
    ...
    def recover(self, original_mol):
        """Recover the label for this node from the original molecule."""
        clique = []
    ...
    def assemble(self):
        ...
        new_cands = [
            cand for i, cand in enumerate(cands) if aroma[i] >= 0
        ]
        if len(new_cands) > 0:
            cands = new_cands
...
class MolTree(object):
    """Tree representation of a molecule decomposed into cliques."""

    def __init__(self, smiles):
        """Initialize molecular tree from SMILES string."""
        self.smiles = smiles
    ...
    def recover(self):
        """Recover labels for all nodes in the tree."""
        for node in self.nodes:
            node.recover(self.mol)

    def assemble(self):
        """Assemble candidate molecules for all nodes in the tree."""
        for node in self.nodes:
            node.assemble()


def dfs(node, fa_idx):
    """Depth-first search to find maximum depth of tree."""
    max_depth = 0
