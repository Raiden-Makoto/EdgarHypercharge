...
def set_atommap(mol, num=0):
    """Set atom map number for all atoms in the molecule."""
...
def get_smiles(mol):
    """Convert molecule to Kekulized SMILES string."""
    return Chem.MolToSmiles(mol, kekuleSmiles=True)


def decode_stereo(smiles2D):
    """Decode stereoisomers from 2D SMILES."""
...
def sanitize(mol):
    """Sanitize molecule by converting to SMILES and back."""
    try:
...
def copy_atom(atom):
    """Create a copy of an atom with symbol, formal charge, and atom map."""
...
def tree_decomp(mol):
    """Perform tree decomposition of a molecule into cliques."""
...
def atom_equal(a1, a2):
    """Check if two atoms are equal (same symbol and formal charge)."""
    return (
        a1.GetSymbol() == a2.GetSymbol() and
...
def ring_bond_equal(b1, b2, reverse=False):
    """Check if two ring bonds are equal."""
...
def attach_mols(ctr_mol, neighbors, prev_nodes, nei_amap):
    """Attach neighbor molecules to the center molecule."""
...
def local_attach(ctr_mol, neighbors, prev_nodes, amap_list):
    """Locally attach neighbors to center molecule using atom mapping."""
...
def enum_attach(ctr_mol, nei_node, amap, singletons):
    ...
    ctr_atoms = [
        atom for atom in ctr_mol.GetAtoms()
        if atom.GetIdx() not in black_list
    ]
...
def enum_assemble(node, neighbors, prev_nodes=[], prev_amap=[]):
    """Enumerate all possible assembly configurations."""
    all_attach_confs = []
    singletons = [
        nei_node.nid for nei_node in neighbors + prev_nodes
    ...
    for amap in all_attach_confs:
        ...
        if (smiles in cand_smiles or
                check_singleton(cand_mol, node, neighbors) is False):
            continue
        cand_smiles.add(smiles)
...
def check_singleton(cand_mol, ctr_node, nei_nodes):
    """Check if candidate molecule has valid singleton configuration."""
    rings = [
        node for node in nei_nodes + [ctr_node]
        if node.mol.GetNumAtoms() > 2
    ]
    singletons = [
        node for node in nei_nodes + [ctr_node]
        if node.mol.GetNumAtoms() == 1
    ]
...
def check_aroma(cand_mol, ctr_node, nei_nodes):
    """Check aromaticity score for candidate molecule."""
    rings = [
        node for node in nei_nodes + [ctr_node]
        if node.mol.GetNumAtoms() >= 3
    ]
    if len(rings) < 2:
    ...
    n_aroma_atoms = 0
    for atom in cand_mol.GetAtoms():
        if (atom.GetAtomMapNum() in benzynes + penzynes and
                atom.GetIsAromatic()):
            n_aroma_atoms += 1
...
def dfs_assemble(cur_mol, global_amap, fa_amap, cur_node, fa_node):
    """Depth-first search assembly of molecule tree."""
    ...
    cand_smiles = list(cand_smiles)
    cand_amap = list(cand_amap)
    
    if cur_node.label not in cand_smiles:
        return
